...
//itemid,itemname ,itemprice ,itemdesc ,itemtype
  Router.post("/add/:id",(req,res)=>{
   ...
   conn.query(sql,value,function (error, results, fields) {
       if (error) throw error;
       return res.status(200).json({"itemid":results.insertId})
     });
  });
...
///everfino/rest_menu/modify/:id and body have id
  Router.put("/modify/:id",(req,res)=>{
    var sql="update menu_"+req.params.id+" set itemname=?,itemprice=?,itemdesc=?,itemtype=? where itemid=?";
    var value=[req.body.itemname,req.body.itemprice,req.body.itemdesc,req.body.itemtype,req.query.itemid]
...
///everfino/rest_menu/:id and body have id
  Router.delete("/delete/:id",(req,res)=>{
   ...
   conn.query(sql,[req.query.itemid],function (error, results) {
       if (error) throw error;
       return res.status(200).json(results)
     });
  });
